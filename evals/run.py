...
async def eval_tools() -> list:
    ...
    asleep.llm.api_key = ""  # tools only
    read = await asleep._run_tool("get_vehicle", {})
    rows.append(
        {
    ...
    woke = await asleep._run_tool("wake_vehicle", {})
    rows.append(
        {
            "id": "wake-sets-online",
    ...
    pending = await asleep._run_tool("unlock_doors", {"confirmed": False})
    rows.append(
        {
    ...
    done = await asleep._run_tool("unlock_doors", {"confirmed": True})
    rows.append(
        {
